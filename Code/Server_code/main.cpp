...
void PktRequest(struct RequestPkt* t, char* s)
{
	...
	strcat(s, "|");            //request package format: "|type|number|content|"
}

//Function to package message
//Format: "|msg|order|IP|port|content|"
//If order and port is empty, format : "|msg||IP||content|"
...
//For the reason the request package format : "|type|number|content|"
...
//For the reason the request package format : "|msg|order|IP|port|content|"
//msg in s[1]
...
//0 means msg type
// format : "|0||time||year/month/day hour:minute:second|"
void getTime(char* s)
{
	time_t t;
	struct tm* lt;
...
//0 means msg type
// format : "|0||list|total clients|client 1's order*client 1's address*client 1's port\nclient 2's ...|"
...
void forwardMessage(char *buffer, SOCKET srcSocket) {
	...
	//"|0||msg||N|" (for failed)  "|0||msg||Y|"  (for success)
...
DWORD WINAPI ProcessClientRequests(LPVOID lpParam) {
	...
	char buffer[256000] = { 0 };    //response string
	int ret;
	...
	while (1)
	{
		...
		//disconnect request//break the thread
		if (_flag == 0)break;
	}
